...
// What I need:
// - The path /Applications/App Store.app/Contents/Resources/AppIcon.icns to get the ICON
// - The app name -> ex. App Store
...
func filterToApplicationsOnly(of strings: [URL]) -> [URL] {
	var newArray = [URL]()
...
func checkIfContentsResourcesPathExists(path: String) -> Bool {
...
func appendIconPath(urls: [URL]) -> [URL] {
...
extendedPaths.count
// 4.-2 get iconFolder *************************************************************
func getContentsOf(folder: URL) -> [URL] {
	let fileManager = FileManager.default
	do {
		let contents = try fileManager.contentsOfDirectory(atPath: folder.path)
		let urls = contents.map { return folder.appendingPathComponent($0) }
...
// 4.-1 get iconFolder *************************************************************
func findIconFile(urls: [URL]) -> [URL] {
	var results = [URL]()
	for url in urls {
...
// 4 get iconFolder *************************************************************
func getIconFolder(urls: [URL]) -> [URL] {
	var iconFolders = [URL]()
	for url in urls {
		let contents = getContentsOf(folder: url)
...
let iconFolders = getIconFolder(urls: extendedPaths)
for folder in iconFolders {
	print(folder)
}
iconFolders.count
// 6 Convert to Path ******************************************************
func urlToPath(urls: [URL]) -> [String] {
	var paths = [String]()
	for url in urls {
		let path = url.path
		paths.append(path)
...
let paths = urlToPath(urls: iconFolders)
// 7.-1.-1 URL to path ********REGEX**************************************
// 7.-1 Get AppName ********REGEX**************************************
func getAppName(url: URL) -> [String] {
...
let stringTest = URL(fileURLWithPath: "/Applications/Icon Set Creator.app/Contents/Resources/AppIcon.icns")
let appNameTest = getAppName(url: stringTest)
